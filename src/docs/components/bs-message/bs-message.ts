...
const message:MessageFn & Partial<Message> & {_context: AppContext|null} = function (options = {} as any, context?: AppContext | null) {
  ...
  if (options.grouping && (text == 0 || text)) {
    let tempQueue: MessageQueueItem | undefined = bsMessageInstanceQueue.find(queueItem => {
      return queueItem.vm.component && queueItem.vm.component.props.message == text;
    });
    if (tempQueue && tempQueue.vm.component) {
  ...
  if (!offsetTop && bsMessageInstanceQueue.length > 0) {
    ...
    offsetTop = (lastMessageVM.component && lastMessageVM.component.props as any).offsetTop || 0;
...
export function closeAllMessage (): void {
  for (let i = bsMessageInstanceQueue.length - 1; i >= 0; i--) {
    if (bsMessageInstanceQueue[i].vm.component) {
      // @ts-ignore
...
export function closeMessage (id: string, userOnClose?: (vm: VNode) => void): void {
  ...
  try {
    (deletedItem.vm.component && deletedItem.vm.component.proxy as any).close();
  } catch (e) {
    console.error(e);
  } finally {
    let deletedItemHeight = ((deletedItem.vm.el && deletedItem.vm.el.offsetHeight) || 0) + offsetTopBase;
    ...
    for (let i = index, len = bsMessageInstanceQueue.length; i < len; i++) {
      ...
      (deletedItem.vm.component && deletedItem.vm.component.proxy as any).startTimer();
